...
def plot_profile(grid, concentartion, color="r", title="Concentration profile"):
    plt.figure()
    plt.plot(grid, concentartion,color)
    plt.xlabel("x")
    plt.ylabel("C")
    plt.title(title)

def solve_1d_diffusion(concentartion,grid_spacing=1.0,time_step=1.0,diffusivity=1.0):
    centered_difference=np.roll(concentartion,-1)-2*concentartion+np.roll(concentartion,1)
